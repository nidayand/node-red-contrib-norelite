...
class Message {
  constructor(node, msg = null, retainId = true) {
    ...
    */
    ...
    /*
    priority defines how a message should be treated.
    ...
    */
    ...
    /*
    transition sets time for transitioning between states
    */
  ...
  fromMessageObject(msg, retainId = true) {
    ...
    if (!(msg.payload.hasOwnProperty('id') && msg.payload.hasOwnProperty('enabled') &&
        msg.payload.hasOwnProperty('priority') && msg.payload.hasOwnProperty('dim')
        && msg.payload.hasOwnProperty('color'))){
      return false;
    }
    if (!retainId)
      this.id = msg.payload.id;
    this.enabled = msg.payload.enabled;
    this.priority = msg.payload.priority;
    this.dim = msg.payload.dim;
    this.color = msg.payload.color;
    if (typeof msg.payload.transition != 'undefined')
      this.transition = msg.payload.transition;
  ...
  toMessageObject(msg) {
    ...
    // Add transition only if set
    if (this.transition != null)
      msg.payload.transition = this.transition;
    return msg;
  }
  ...
  /**
   * Retrieves the transition setting
   * @author nidayand
   * @returns {number} Transition value
   */
  getTransition(){
  ...
  /**
   * Compares values. I.e. enabled, dim, color
   * @author nidayand
   * @param   {object}  obj Message object to compare with
   * @returns {boolean} Returns true if the values are the same
   */
  compareTo(obj) {
    if (this.is_enabled() != obj.is_enabled() || this.getDim() != obj.getDim() || this.getColor() != obj.getColor() || this.getPriority() != obj.getPriority() || this.getTransition() != obj.getTransition()) {
      return false;
    } else {
      return true;
    }
