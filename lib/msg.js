...
class Message {
  ...
  fromMessageObject(msg, retainId = true) {
    if (!(msg.payload.hasOwnProperty('id') && msg.payload.hasOwnProperty('enabled') &&
        msg.payload.hasOwnProperty('type') && msg.payload.hasOwnProperty('dim')
   ...
   */
  compareTo(obj) {
    if (this.is_enabled() != obj.is_enabled() || this.getDim() != obj.getDim() || this.getColor() != obj.getColor() || this.getType() != obj.getType()) {
      return false;
    } else {
      return true;
    }
